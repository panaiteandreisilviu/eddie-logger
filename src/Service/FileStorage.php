...
class FileStorage
{
    ...
    {
        return $this->config->logFilesLocation . DIRECTORY_SEPARATOR . "e_$channel.html";
    }
    ...
    public function addSfDumpAssets(string $logFileFullPath): void
    {
        ...
        if (!file_exists($this->config->sfDumpAssetsLocation . 'sfdump.js')) {
            return;
        }
        if (!file_exists($this->config->sfDumpAssetsLocation . 'sfdump.css')) {
            return;
        }

        $sfDumpJs = "<script>" . file_get_contents($this->config->sfDumpAssetsLocation . 'sfdump.js') . "</script>";
        $sfDumpCss = "<style>" . file_get_contents($this->config->sfDumpAssetsLocation . 'sfdump.css') . "</style>";
